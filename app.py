...
try:
    ...
    st.write(f"Current Locale: {current_locale}")
    st.write(f"Default Locale: {default_locale}")
...
if st.session_state["view_budget"]:
    ...
    locale.setlocale(locale.LC_ALL, 'en_US.UTF-8')  # Set locale to display commas in numbers
    budget_value = st.selectbox(
        "What is your budget?", 
        ["Select here"] + [locale.format_string("%d", x, grouping=True) for x in range(400000, 1000001, 200000)],
